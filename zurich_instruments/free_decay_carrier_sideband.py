...
class FreeDecayCarrierSidebandProcedure(Procedure):
    """Legacy carrier + single-sideband free decay procedure."""
    ...
    use_current_sample_rates = BooleanParameter(
        "Use current demod sample rates", default=False
    )
    ...
    carrier_k = FloatParameter(
        "Carrier k constant", units="1/V", default=3931072.9988525705
    )
    carrier_V0 = FloatParameter(
        "Carrier drive that k was obtained at", units="V", default=300e-6
    )
    sideband_k = FloatParameter(
        "Sideband k constant", units="1/V", default=93260373.7208108
    )
    ...
    def startup(self):
        ...
        self.pre_drop_wait = (
            self.settle_after_set if changed_anything else self.delay_before_drop
        )
    ...
    def _validate_parameters(self):
        ...
        if min(
            ...
            raise ValueError(
                "Demodulator and oscillator numbers are 1-based and must be > 0."
            )
        if not self.use_current_carrier_drive:
        ...
        if not self.use_current_sample_rates:
            if self.demod_sample_rate is None or self.demod_sample_rate <= 0:
                raise ValueError(
                    "demod_sample_rate must be > 0 when not using current values."
                )
        if not self.use_current_sideband_drive:
        ...
        if not self.use_current_time_constants:
            if self.carrier_timeconstant is None or self.carrier_timeconstant <= 0:
                raise ValueError(
                    "carrier_timeconstant must be > 0 when not using current values."
                )
            if self.sideband_timeconstant is None or self.sideband_timeconstant <= 0:
                raise ValueError(
                    "sideband_timeconstant must be > 0 when not using current values."
                )
    ...
    def _set_carrier_drive(self, amplitude: float, frequency: float):
        self.daq.setDouble(f"/{self.zur_id}/mods/0/carrier/amplitude", float(amplitude))
        self.daq.setDouble(
            f"/{self.zur_id}/oscs/{self.carrier_osc_index}/freq", float(frequency)
        )
        self.daq.sync()

    def _set_sideband_drive(self, amplitude: float, frequency: float):
        self.daq.setDouble(
            f"/{self.zur_id}/mods/0/sidebands/1/amplitude", float(amplitude)
        )
        self.daq.setDouble(
            f"/{self.zur_id}/oscs/{self.sideband_osc_index}/freq", float(frequency)
        )
        self.daq.sync()
    ...
    def _set_timeconstant(self, demod_index: int, value: float):
        self.daq.setDouble(
            f"/{self.zur_id}/demods/{demod_index}/timeconstant", float(value)
        )
        ...
        if carrier_timestamps.size and drop_device_ts is not None:
            ...
            if times is None:
                if carrier_timestamps.size:
                    times = (carrier_timestamps - carrier_timestamps[0]) / float(
                        self.clockbase
                    )
                else:
...
class FreeDecayCarrierSidebandWindow(ManagedDockWindow):
    def __init__(self):
        carrier_plot = PlotWidget(
            name="Carrier X vs time",
            columns=FreeDecayCarrierSidebandProcedure.DATA_COLUMNS,
            x_axis="t_rel",
            y_axis="carrier_X",
        )
        ...
        super().__init__(
            procedure_class=FreeDecayCarrierSidebandProcedure,
            inputs=FreeDecayCarrierSidebandProcedure.PARAMETERS,
            displays=FreeDecayCarrierSidebandProcedure.PARAMETERS,
            x_axis=["t_rel"],
        ...
        self.setWindowTitle("Zurich Carrier + Sideband Free Decay")
        self.filename_prefix = "free-decay-carrier-sideband"
    ...
    def _default_data_dir() -> Path:
        ...
        if preferred.exists():
            return preferred
        fallback = (
            Path(__file__).resolve().parent.parent / "data-files/free-decay-sideband"
        )
...
if __name__ == "__main__":
    ...
    window = FreeDecayCarrierSidebandWindow()
    window.show()
    sys.exit(app.exec_())
