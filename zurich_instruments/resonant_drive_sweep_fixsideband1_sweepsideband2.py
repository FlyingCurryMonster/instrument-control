...
class ResonantDriveSweepFixSideband1SweepSideband2Procedure(Procedure):
    """Sweep sideband 2 drive while retuning sideband 2 and holding sideband 1 fixed."""
    ...
    sideband2_start_drive = FloatParameter("Sideband 2 start drive", units="V", default=0.1e-3)
    sideband2_end_drive = FloatParameter("Sideband 2 end drive", units="V", default=3e-3)
    ...
    sideband1_fixed_target = FloatParameter(
        "Fixed sideband 1 target fc-fm1",
        units="Hz",
        default=645.702,
        decimals=10,
        ui_class=HighPrecisionScientificInput,
    )
    sideband2_initial_target = FloatParameter(
        "Initial sideband 2 target fc+fm2",
        units="Hz",
        default=1320.93209,
        decimals=10,
        ui_class=HighPrecisionScientificInput,
    ...
    phase_band = FloatParameter("Sideband 2 phase band", units="deg", default=6.0)
    ...
    file_prefix = Parameter("File prefix", default="resonant_drive_sweep_fixsideband1_sweepsideband2")
    ...
    PARAMETERS = [
        ...
        "carrier_drive",
        "carrier_frequency",
        "sideband2_start_drive",
        "sideband2_end_drive",
        "num_points",
        "logspace",
        "use_drive_freq_csv",
        "drive_freq_csv_path",
        "sideband2_initial_target",
        "sideband1_fixed_target",
        "sideband1_drive",
        "reverse_sweep",
        "retune_down_sweep",
        "phase_band",
    ...
    def startup(self):
        log.info("Starting resonant sideband 2 drive sweep with fixed sideband 1")
        self._validate_parameters()
        self.current_sideband1_target = float(self.sideband1_fixed_target)
    ...
    def execute(self):
        current_target = float(self.sideband2_initial_target)
    ...
    def _tune_and_measure(
        ...
        while True:
            ...
            if use_existing_state:
                ...
                self._set_sideband2_drive(drive, current_target)
            ...
            in_band = abs(measurement["sideband2_phase"]) <= self.phase_band
            fixed_sideband_phase = float(measurement["sideband1_phase"])
            ...
            measurement.update(
                {
                    ...
                    "sideband2_drive_set": float(drive),
                    "f_carrier_set": float(self.carrier_frequency),
                    "f_sideband1_target_set": float(self.current_sideband1_target),
                    "f_sideband1_osc_set": float(self._sideband1_osc_freq()),
                    "f_sideband2_target_set": float(current_target),
                    "f_sideband2_osc_set": float(
                        self._sideband2_osc_freq_for_target(current_target)
                    ),
                    "in_band": int(in_band),
            ...
            if not in_band and retune and iterations < self.max_iterations:
                f0_infer = measurement["sideband2_f0_infer"]
            ...
            if (
                ...
                self.current_sideband1_target = float(
                    self.current_sideband1_target + fixed_frequency_correction
                )
    ...
    def _set_fixed_channels(self):
        ...
        self.daq.setDouble(
            ...
            float(self.sideband1_drive),
        )
        self.daq.setDouble(self._freq_path(self.sideband1_osc_index), float(self._sideband1_osc_freq()))
        self.daq.sync()

    def _set_sideband2_drive(self, amplitude: float, target_frequency: float):
        self._validate_safe_amplitude("sideband2_drive", amplitude)
        self.daq.setDouble(
        ...
        self.daq.setDouble(
            ...
            float(self._sideband2_osc_freq_for_target(target_frequency)),
        )
        self.daq.sync()
    ...
    def _apply_current_sideband1_target(self) -> None:
        self.daq.setDouble(
            ...
            float(self._sideband1_osc_freq()),
        )
        self.daq.sync()
    ...
    def _update_last_tau(self, measurement: dict) -> None:
        tau = measurement.get("sideband2_tau_infer")
    ...
    def _build_drive_points(self) -> np.ndarray:
        if self.logspace:
            return np.logspace(
                np.log10(self.sideband2_start_drive),
                np.log10(self.sideband2_end_drive),
                int(self.num_points),
            )
        return np.linspace(
            self.sideband2_start_drive,
            self.sideband2_end_drive,
            int(self.num_points),
        )
    ...
    def _load_drive_freq_csv(self) -> Tuple[List[float], List[float]]:
        ...
        with path.open("r", newline="") as handle:
            ...
            if reader.fieldnames is None:
                raise ValueError("CSV must include headers: drive_voltage, sideband2_target.")
            required = {"drive_voltage", "sideband2_target"}
            if not required.issubset(set(reader.fieldnames)):
                raise ValueError("CSV headers must include drive_voltage and sideband2_target.")
            for row in reader:
                drive = float(str(row.get("drive_voltage", "")).strip())
                target = float(str(row.get("sideband2_target", "")).strip())
                self._validate_safe_amplitude("csv drive_voltage", drive)
                self._sideband2_osc_freq_for_target(target)
                drives.append(drive)
    ...
    def _validate_parameters(self) -> None:
        ...
        if not self.use_drive_freq_csv:
            self._validate_safe_amplitude("sideband2_start_drive", self.sideband2_start_drive)
            self._validate_safe_amplitude("sideband2_end_drive", self.sideband2_end_drive)
            if self.logspace and (
                self.sideband2_start_drive <= 0 or self.sideband2_end_drive <= 0
            ):
        ...
        if self.sideband2_initial_target <= 0:
            raise ValueError("Initial sideband 2 target must be > 0.")
        if self.sideband1_fixed_target <= 0:
            raise ValueError("Fixed sideband 1 target must be > 0.")
        if min(
        ...
        self._validate_safe_amplitude("carrier_drive", self.carrier_drive)
        self._validate_safe_amplitude("sideband1_drive", self.sideband1_drive)
        self._sideband1_osc_freq()
        self._sideband2_osc_freq_for_target(self.sideband2_initial_target)
    ...
    def _sideband1_osc_freq(self) -> float:
        target_frequency = float(
            getattr(self, "current_sideband1_target", self.sideband1_fixed_target)
        )
        osc_freq = float(self.carrier_frequency) - target_frequency
        if osc_freq < 0:
            raise ValueError(
                "Fixed sideband 1 target %.10f Hz is above carrier %.10f Hz, producing negative fm1."
                % (target_frequency, self.carrier_frequency)
            )
        return osc_freq

    def _sideband2_osc_freq_for_target(self, target_frequency: float) -> float:
        osc_freq = float(target_frequency) - float(self.carrier_frequency)
        if osc_freq < 0:
            raise ValueError(
                "Sideband 2 target %.10f Hz is below carrier %.10f Hz, producing negative fm2."
                % (target_frequency, self.carrier_frequency)
            )
        return osc_freq
...
class ResonantDriveSweepFixSideband1SweepSideband2Window(ManagedDockWindow):
    def __init__(self):
        drive_plot = PlotWidget(
            name="Sideband 2 Drive Sweep",
            columns=ResonantDriveSweepFixSideband1SweepSideband2Procedure.DATA_COLUMNS,
            x_axis="sideband2_drive_set",
            y_axis="sideband2_f0_infer",
        )
        ...
        q_plot = PlotWidget(
            ...
            columns=ResonantDriveSweepFixSideband1SweepSideband2Procedure.DATA_COLUMNS,
            x_axis="sideband2_drive_set",
            y_axis="sideband2_Q_infer",
        )
        sideband2_plot = PlotWidget(
            name="Sideband 1 Diagnostics",
            columns=ResonantDriveSweepFixSideband1SweepSideband2Procedure.DATA_COLUMNS,
            x_axis="sideband2_drive_set",
            y_axis="sideband1_R",
        )
        ...
        super().__init__(
            procedure_class=ResonantDriveSweepFixSideband1SweepSideband2Procedure,
            inputs=ResonantDriveSweepFixSideband1SweepSideband2Procedure.PARAMETERS,
            displays=ResonantDriveSweepFixSideband1SweepSideband2Procedure.PARAMETERS,
            x_axis=["sideband2_drive_set"],
            y_axis=[
                "sideband2_f0_infer",
                "sideband2_phase",
        ...
        self.setWindowTitle("Zurich Resonant Drive Sweep Fix Sideband1 Sweep Sideband2")
...
if __name__ == "__main__":
    ...
    window = ResonantDriveSweepFixSideband1SweepSideband2Window()
    window.show()
    sys.exit(app.exec_())
