...
class ResonantDriveSweepFixSideband1SweepSideband2Procedure(Procedure):
            ...
            measurement.update(
                {
                    ...
                    "f_sideband1_target_set": float(self.sideband1_fixed_target),
    ...
    def _sideband1_osc_freq(self) -> float:
        osc_freq = float(self.carrier_frequency) - float(self.sideband1_fixed_target)
        if osc_freq < 0:
            raise ValueError(
                "Fixed sideband 1 target %.10f Hz is above carrier %.10f Hz, producing negative fm1."
                % (self.sideband1_fixed_target, self.carrier_frequency)
            )
        return osc_freq
